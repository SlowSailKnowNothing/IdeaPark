...
async function main() {
  ...
  for (const agent of builtinAgents) {
    await prisma.agent.upsert({
      where: { id: agent.id },
      update: { name: agent.name, description: agent.description, systemPrompt: agent.systemPrompt, capabilities: agent.capabilities, avatarEmoji: agent.avatarEmoji },
      create: {
        ...agent,
        modelName: 'gpt-4',
        visibility: 'public',
  ...
  if (existingIdeas === 0) {
    ...
    console.log(`Skipping ideas — ${existingIdeas} already exist.`)
  }
